...
export class MicrobitStateButtonB extends MicrobitStateButton {

    protected async serviceUpdate(service: ButtonService) {
        await service.addEventListener("buttonbstatechanged", event => this.setBackground(event.detail));
        this.setBackground(await service.readButtonBState());
    }
}
